...
export async function fetchWords(): Promise<Word[]> {
  const { data } = await api.get<Word[]>('/v1/admin/words');
  return data;
}

export async function createWord(payload: WordPayload): Promise<Word> {
  const { data } = await api.post<Word>('/v1/admin/words', payload);
  return data;
}

export async function updateWord(id: number, payload: WordPayload): Promise<Word> {
  const { data } = await api.put<Word>(`/v1/admin/words/${id}`, payload);
  return data;
}
